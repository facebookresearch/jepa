...
def sync_wait(scene, t0: float, dur: float, label: str = "") -> None:
    ...
    if remaining > 0.05:
        ...
        print(f"  ⚠  [{label}] animation ({elapsed:.1f}s) > audio ({dur:.1f}s)")
...
class Ch5Scene(Scene):
    ...
    def _ch5_1_loss_stopgrad(self, is_master: bool = False):
        ...
        # Slide formula to corner; clear title
        self.play(
        ...
        # Forward-pass arrows (white/dim — forward data flow)
        ctx_rect  = ctx_grp[0]
        ...
        # ── Phase 4: Gradient backpropagation ────────────────────────────
        grad_note = Text("∇  gradient flows backward", font_size=16, color=C_YELLOW)
        ...
        stop_lbl.next_to(stop_wall, RIGHT, buff=0.14).set_z_index(6)
    ...
    def _ch5_2_collapse_ema(self, is_master: bool = False):
        ...
        # ── Phase 5: Momentum schedule line ──────────────────────────────
        ...
        # ── Phase 6: MAD theorem ──────────────────────────────────────────
        mad_ctx = Text(
            "When Predictor is optimal under L₁ loss:",
            font_size=20, color=C_TEXT,
        ).next_to(ema_eq, DOWN, buff=0.42)
        mad_eq1 = MathTex(
...
class Ch6Scene(Scene):
    ...
    def _ch6_1_benchmarks(self, is_master: bool = False):
        ...
        ssv2_cols  = [C_ORANGE, C_DIM, C_DIM, C_DIM, C_DIM]
        ssv2_chart = BarChart(
            values=ssv2_vals, bar_names=ssv2_names,
            y_range=[0, 80, 20], x_length=5.2, y_length=3.0,
        ...
        k400_cols  = [C_ORANGE, C_DIM, C_DIM]
        k400_chart = BarChart(
            values=k400_vals, bar_names=k400_names,
            y_range=[0, 90, 30], x_length=3.4, y_length=3.0,
    ...
    def _ch6_2_efficiency(self, is_master: bool = False):
        ...
        # ── Phase A: Controlled experiment boxes ─────────────────────────
        ctrl_lbl = Text(
        ...
        eff_cols  = [C_ORANGE, C_DIM, C_DIM, C_DIM]
        eff_chart = BarChart(
...
class Ch7Scene(Scene):
    ...
    def _ch7_attentive_probing(self, is_master: bool = False):
        ...
        lock_grp.next_to(frozen_rect, UR, buff=-0.1).shift(LEFT * 0.1 + DOWN * 0.12).set_z_index(5)
        ...
        pool_lbl = Text("Pooled\nrepr.", font_size=13, color=C_PURPLE)
        pool_grp = VGroup(pool_dot, pool_lbl).arrange(DOWN, buff=0.1)
        ...
        # ── Phase 5: Multi-task downstream results ────────────────────────
        res_title = Text(
        ...
        # ── Phase 6: Conclusion bullets ───────────────────────────────────
        concl_title = Text(
            "V-JEPA — Key Takeaways",
            font_size=34, color=C_YELLOW, weight=BOLD,
        ).to_edge(UP, buff=0.48)
        self.play(FadeIn(concl_title, shift=DOWN * 0.2), run_time=0.7)

        bullets = VGroup(
            Text("①  Predict in latent space — not pixels",                          font_size=22, color=C_TEXT),
            Text("②  3D Multi-Block masking prevents temporal information shortcuts", font_size=22, color=C_TEXT),
            Text("③  Stop-gradient + EMA = stable, collapse-free training",          font_size=22, color=C_TEXT),
            Text("④  Frozen evaluation reveals true representation quality",          font_size=22, color=C_TEXT),
            Text("⑤  210M samples — ~200× more efficient than OpenCLIP",             font_size=22, color=C_YELLOW),
...
class VJEPACh567Scene(Ch5Scene, Ch6Scene, Ch7Scene):
    ...
    def construct(self):
        ...
        # Single master audio track covering the entire video
        master_audio = "media/ch567_master_audio.wav"
        if os.path.exists(master_audio):
            self.add_sound(master_audio)

        # Chapter 5 — Loss, Stop-Gradient, Collapse, EMA
        self._ch5_1_loss_stopgrad(is_master=True)
        self._ch5_2_collapse_ema(is_master=True)

        # Chapter 6 — Benchmarks, Sample Efficiency
        self._ch6_1_benchmarks(is_master=True)
        self._ch6_2_efficiency(is_master=True)

        # Chapter 7 — Attentive Probing, Multi-task Results, Conclusion
        self._ch7_attentive_probing(is_master=True)
