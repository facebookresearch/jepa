...
def main(args_eval, resume_preempt=False):
    ...
    if rank == 0:
        csv_logger = CSVLogger(
            log_file, ("%d", "epoch"), ("%.5f", "loss"), ("%.5f", "acc")
        )
...
def run_one_epoch(
    ...
    for itr, data in enumerate(data_loader):
        ...
        if attend_across_segments:
            ...
            loss = (
                sum([sum([criterion(ost, labels) for ost in os]) for os in outputs])
                / len(outputs)
                / len(outputs[0])
            )
        with torch.no_grad():
            if attend_across_segments:
                ...
                outputs = (
                    sum([sum([F.softmax(ost, dim=1) for ost in os]) for os in outputs])
                    / len(outputs)
                    / len(outputs[0])
                )
            top1_acc = 100.0 * outputs.max(dim=1).indices.eq(labels).sum() / batch_size
